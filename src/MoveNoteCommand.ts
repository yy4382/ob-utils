...
import {addRecentDirectory, checkFileExists, cleanupRecentDirectories} from './utils/directoryUtils';
...
export async function moveNoteToDirectory(
	...
	// Build target path
	const targetPath = targetDir ? `${targetDir}/${activeFile.name}` : activeFile.name;

	// Check for file conflicts
	if (checkFileExists(app.vault, targetDir, activeFile.name)) {
		new Notice(`A file named '${activeFile.name}' already exists in this directory`);
		return;
	}
